...
class TestBadges:
    ...
    def test_chips_uniform_width_with_full_words(self):
        # 4-letter levels center perfectly; 5-letter levels carry the
        # unavoidable half-character offset (extra space on the right)
        assert _strip_ansi(format_level("INFO")) == "  INFO  "
        assert _strip_ansi(format_level("WARNING")) == "  WARN  "
        assert _strip_ansi(format_level("CRITICAL")) == "  CRIT  "
        assert _strip_ansi(format_level("ERROR")) == " ERROR  "
        assert _strip_ansi(format_level("DEBUG")) == " DEBUG  "
